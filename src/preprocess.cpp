...
void Preprocess::oust64_handler( const sensor_msgs::PointCloud2::ConstPtr &msg )
{
    ...
    else
    {
        ...
        for ( int i = 0; i < pl_orig.points.size(); i++ )
        {
            ...
            added_pt.y = pl_orig.points[ i ].y;
            added_pt.z = pl_orig.points[ i ].z;
            if ( 1 ) // ANCHOR: remove nearby points
            {
                pt_vec = Eigen::Vector3d( added_pt.x, added_pt.y, added_pt.z * 0 );
                if ( pt_vec.norm() < 0.5 )
                {
                    continue;
                }
    ...
    // std::cout << "feat size:" << pl_surf.points.size() << std::endl;
    // pub_func(pl_surf, pub_full, msg->header.stamp);
    // pub_func(pl_surf, pub_corn, msg->header.stamp);
}
...
//         double time_stamp = msg->header.stamp.toSec();
//         cout << "===================================" << endl;
//         std::cout << "time stamp unit scale:" << time_unit_scale << std::endl;
//         printf( "Pt size = %d, N_SCANS = %d\r\n", plsize, N_SCANS );
//         for ( int i = 0; i < pl_orig.points.size(); i++ )
//         {
//             if ( i % point_filter_num != 0 )
...
void Preprocess::velodyne32_handler( const sensor_msgs::PointCloud2::ConstPtr &msg )
{
    pl_surf.clear();
    pl_corn.clear();
    pl_full.clear();
